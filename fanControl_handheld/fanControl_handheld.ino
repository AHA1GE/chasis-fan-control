...
#define BATT_CRITICAL_BLINK_HZ 5    // battery too low or too high: 5 Hz to indicate error
#define BATT_LOW_BLINK_HZ 2         // Low-battery warning: 2 Hz red blink overrides normal display
...
// Battery
static int g_batt_percent = 0;        // 0–100
static bool g_batt_critical = false;  // too low or too high
 ...
 * Battery ADC (1S only)
 * ================================================================ */
static void batt_task(unsigned long nowMillis) {
  ...
  static int buf[4] = { 0 };
  static int idx = 0;
  static int warmup = 0;  // count samples until buffer is full
  buf[idx] = raw;
  idx = (idx + 1) & 3;
  if (warmup < 4) warmup++;
  ...
  // Skip critical check until the rolling-average buffer has 4 real samples.
  // Without this, the first few readings average against trailing zeros and
  // falsely read far below BATT_MIN, latching g_batt_critical permanently.
  if (warmup >= 4 && (vBatt < BATT_MIN || vBatt > BATT_MAX)) {
    fan_off();
    g_batt_critical = true;       // set latch, need re-init(hard reset or sleep-wake) to clear
    if (g_state != S_CHARGING) {  // charging always takes highest priority
      g_state = S_WAIT;
      g_tWait = nowMillis;  // restart auto-sleep timer so critical blink is visible for 5 s
    }
  }
  ...
  if (vBatt <= BATT_LOW) {
    ...
    g_batt_percent = (int)((100UL * (vBatt - BATT_LOW)) / (BATT_FULL - BATT_LOW));
  }
  if (g_batt_percent > 100) {
    g_batt_percent = 100;
  }
  if (g_batt_percent < 0) {
    g_batt_percent = 0;
...
static void btn_task(unsigned long nowMillis) {
  ...
  if (clicks && (nowMillis - tRelease) > BTN_DOUBLE_GAP_MS) {
    ...
    if (g_state == S_ON) {
      ...
      if (clicks == 1) {  // single: ON at FAN_LVL_1%
        g_dutyIdx = 0;
        fan_pwm_write(dutyTable[0]);
      } else {  // double: ON at 100 % (quick shortcut)
        g_dutyIdx = FAN_LVL_COUNT - 1;
        fan_pwm_write(dutyTable[FAN_LVL_COUNT - 1]);
      }
...
static void led_task(unsigned long nowMillis) {
  ...
  if (g_batt_critical && (g_state == S_WAIT || g_state == S_ON)) {
    if ((nowMillis % (1000 / BATT_CRITICAL_BLINK_HZ)) < (500 / BATT_CRITICAL_BLINK_HZ))  // Battery critical
    {
  ...
  } else if (g_batt_percent < 10 && (g_state == S_WAIT || g_state == S_ON))  // Battery low
  {
    if ((nowMillis % (1000 / BATT_LOW_BLINK_HZ)) < (500 / BATT_LOW_BLINK_HZ)) {
      // Red on (GRB order: G=0, R=255, B=0)
      g_led_G = 0;
      g_led_R = 255;
      g_led_B = 0;
      ...
      case S_CHARGING:
        {
          if (g_batt_percent >= 100) {
            ...
            g_led_R = ((100 - g_batt_percent) * 150 / 100);
            g_led_G = (g_batt_percent * 150 / 100);
            g_led_B = (0);
          }
          break;
        }
...
void loop(void) {
  ...
  if (g_state == S_CHARGING && chg_high) {
    fan_off();
    g_state = S_WAIT;
    g_tWait = nowMillis;  // restart auto-sleep timer
    g_led_dirty = true;
  }
